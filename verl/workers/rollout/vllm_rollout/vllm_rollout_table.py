...
class PythonVLLMRollout(BaseRollout):
    ...
    def generate_sequences(self, prompts: DataProto, **kwargs) -> DataProto:
        ...
        # Prepare for Python code processing iterations
        current_prefix_list = []
        ...
        pber = tqdm(range(self.max_action_iterations + 1), desc="Processing Python code blocks...", disable=False)
        for iter in pber:
            ...
            for i, response_str in enumerate(response_str_list):
                has_python_code = "```python" in response_str
                if has_python_code:
                    print(f"DEBUG: Sample {i} contains Python code:")
                    code_match = self.python_code_pattern.search(response_str)
                    if code_match:
                        print(f"  Python code content: {code_match.group(1)[:100]}...")
    
                processed_text, has_python_code = self.process_python_code(response_str)
                
                if has_python_code:
                    output_added = "```output" in processed_text
                    print(f"DEBUG: Python code processed for sample {i}, output added: {output_added}")
                    # Update prefix with code execution results
                    new_prefix_list[i] = current_prefix_list[i] + processed_text
                    action_flag_list[i] = True
            
            # Log debug information for Python code execution
            if any(action_flag_list):
                print(f"DEBUG - Python code iteration {iter+1}:")
                for i, (flag, prefix) in enumerate(zip(action_flag_list, new_prefix_list)):
                    if flag:
                        print(f"Sample {i} executed Python code, new prefix length: {len(prefix)}")
            ...
            if any(action_flag_list):
                ...
                if new_prompts_list:
                    ...
                    # Log executed Python code blocks
                    if any(action_flag_list):
                        print("Executed Python code blocks:")
                        for i, flag in enumerate(action_flag_list):
                            if flag:
                                print(f"Sample {i}: Executed code, response: {response_str_list[i][:100]}...")
            else:
