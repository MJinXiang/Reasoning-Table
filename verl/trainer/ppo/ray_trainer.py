...
class RayPPOTrainer(object):
    ...
    def _validate(self):
        data_source_lst = []
        direct_metrics_dict = defaultdict(lambda: defaultdict(list))  
        ...
        for test_data in self.val_dataloader:
            ...
            data_sources = test_batch.non_tensor_batch.get('data_source', ['unknown'] * len(scores))
            data_source_lst.append(data_sources)
            
       
            for i, source in enumerate(data_sources):
                if i < len(scores):
                    direct_metrics_dict[source]['reward'].append(scores[i])
                
                if 'test_metrics' in test_batch.non_tensor_batch:
                    metrics = test_batch.non_tensor_batch['test_metrics']
                    if 'em_scores' in metrics and i < len(metrics['em_scores']):
                        direct_metrics_dict[source]['em'].append(metrics['em_scores'][i])
                    
                    if 'f1_scores' in metrics and i < len(metrics['f1_scores']):
                        direct_metrics_dict[source]['f1'].append(metrics['f1_scores'][i])
                    
                    if 'bleu_scores' in metrics and i < len(metrics['bleu_scores']):
                        direct_metrics_dict[source]['bleu'].append(metrics['bleu_scores'][i])
                    
                    if 'accuracy_scores' in metrics and i < len(metrics['accuracy_scores']):
                        direct_metrics_dict[source]['accuracy'].append(metrics['accuracy_scores'][i])
                    
                    if 'combined_scores' in metrics and i < len(metrics['combined_scores']):
                        direct_metrics_dict[source]['combined'].append(metrics['combined_scores'][i])
        ...
        for data_source, metrics in direct_metrics_dict.items():
            for metric_name, values in metrics.items():
                if values: 
                    metric_dict[f'val/{metric_name}_score/{data_source}'] = np.mean(values)
        
     
        reward_extra_infos_dict = defaultdict(list)
        data_sources = np.concatenate(data_source_lst, axis=0)
        for source, metrics in direct_metrics_dict.items():
            for metric_name, values in metrics.items():
                reward_extra_infos_dict[metric_name].extend(values)
