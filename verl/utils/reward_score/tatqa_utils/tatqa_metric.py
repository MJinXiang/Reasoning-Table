...
def normalize_answer_for_matching(answer):
    """Normalize answer format for matching"""
    if not answer:
        return answer
        
    # Handle negative number format with parentheses $(123) -> -123
    answer = re.sub(r'\$\(([0-9,.]+)\)', r'-\1', answer)
    
    # Handle percentage - only remove % symbol, keep original numeric value
    if isinstance(answer, str) and '%' in answer:
        answer = answer.replace('%', '').strip()
    
    # Remove unit descriptive words
    answer = re.sub(r'million|percent', '', answer)
...
def get_metrics(predicted: Union[str, List[str], Tuple[str, ...]],
    ...
    """
    Improved metric calculation, correctly handles percentages but does not allow numerical errors
    """
    predicted_bags = _answer_to_bags(predicted)
    gold_bags = _answer_to_bags(gold)

    # If F1=1, then set EM=1
    ...
    # Try numerical matching (for arithmetic type answers)
    norm_pred = [normalize_answer_for_matching(p) for p in predicted_bags[0]]
    norm_gold = [normalize_answer_for_matching(g) for g in gold_bags[0]]
    
    # Check if all are numerical, require exact equality
    try:
        pred_nums = [float(p) for p in norm_pred if p]
        gold_nums = [float(g) for g in norm_gold if g]
        
        # Require exact numerical equality (no tolerance for errors)
        if pred_nums[0] == gold_nums[0]:
            return 1.0, 1.0
    except:
        pass  # Conversion failed, continue with default comparison
...
def normalize_multi_span(answer_list):
    """
    Normalize multi-span answers to make them easier to match
    - Improvement: correctly handle numbers with thousand separators
    """
    # If already a list, ensure each element is a string
    if isinstance(answer_list, list):
        return [str(item).strip() for item in answer_list if item]
    
    # If it's a string, try to split
    if isinstance(answer_list, str):
        # First handle cases separated by "and"
        and_parts = re.split(r'\s+and\s+', answer_list)
        
        result = []
        for part in and_parts:
            # Check if it contains commas used as thousand separators
            if re.search(r'\d,\d', part):
                # If it's a number in thousand separator format, keep the entire number
                result.append(part.strip())
            else:
                # Otherwise split by regular separators
                items = re.split(r'\s*,\s*|\s*;\s*|\s*\|\s*', part)
                result.extend([item.strip() for item in items if item.strip()])
        
        return result
    
    # For other cases, return single-element list
...
def extract_gold_answers(qa_annotation):
    ...
    gold_answers = []
    if answer_type in ['multi-span']:
        gold_answers = normalize_multi_span(answer_content)
    elif answer_type in ['span']: 
        if isinstance(answer_content, list):
            gold_answers = [str(item) for item in answer_content]
        else:
