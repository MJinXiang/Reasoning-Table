...
def get_raw_scores(model_results, reference):
    ...
    return collections.OrderedDict(
        [
            ...
            ("total_reference", len(reference['reference'])),  # Total questions in reference answers
        ]
    )
...
def evaluate_model(model_results_file, test_answers_file, evaluate_save_dir, create_eval_format_flag=True):
    ...
    print(f"Actually evaluated questions: {scores['total']} / {scores['total_reference']}")  # Show actual evaluated/total questions
    print("-" * 40)
