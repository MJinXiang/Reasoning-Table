...
def normalize_answer(answer):
    ...
    numeric_with_units_match = re.match(r'^([\d,]+)\s*(days|years|pages|km\/h|mph)$', answer)
    ...
    import unicodedata
    answer = ''.join(c for c in unicodedata.normalize('NFKD', answer)
...
def exact_match_enhanced(prediction, reference):
    ...
    if pred_num_match and ref_units_match and pred_num_match.group(1) == ref_units_match.group(1):
        return 1
...
def evaluate_answers(input_file, output_file=None, verbose=True):
    ...
    try:
        ...
        print(f"Error: Cannot parse input file {input_file}, please ensure it is valid JSON.")
        return None
    ...
    if not isinstance(data, list):
        if isinstance(data, dict):
            ...
            print(f"Error: Input data format is incorrect, should be a list of JSON objects or a single JSON object.")
            return None
