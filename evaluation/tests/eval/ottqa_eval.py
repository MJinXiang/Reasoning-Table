...
def prepare_reference_from_results(model_results_file):
    ...
    for item in model_results:
        question_id = item["question_id"]
        reference["reference"][question_id] = item.get("ground_answer", "")  # Get ground truth from results
...
def get_raw_scores(model_results, reference):
    ...
    return collections.OrderedDict(
        [
            ...
            ("total_reference", len(reference['reference'])),  # Total number of questions in the reference
        ]
    )
...
def evaluate_model(model_results_file, evaluate_save_dir, create_eval_format_flag=True):
    ...
    reference = prepare_reference_from_results(model_results_file)  # Now getting reference from model results
    ...
    print(f"Questions actually evaluated: {scores['total']} / {scores['total_reference']}") # Show actual evaluated/total questions
    print("-" * 40)
