...
def evaluate(examples: List[Dict]) -> Tuple[float, List[Dict]]:
    ...
    for ex in tqdm(examples, desc="Evaluating"):
        ...
        detail = {
            ...
            "model_full_response": ex.get("model_full_response", "")
        }
...
def save_results(bleu_score: float, details: List[Dict], output_file: str, results_file: str):
    """
...
def main():
    parser = argparse.ArgumentParser(description='Evaluate FeTaQA results using BLEU score')
