...
def arithmetic_exact_match(prediction, ground_truth, tolerance=1e-5):
    ...
    rel_tol = min(abs(truth_num) * 0.01, 0.1)  # Allow 1% relative error, but not exceeding 0.1
    abs_tol = min(abs(truth_num) / 1000, 0.1)  # Allow one-thousandth absolute error, but not exceeding 0.1
