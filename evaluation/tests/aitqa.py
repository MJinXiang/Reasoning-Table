...
def prepare_markdown_item(item, markdown_prompts=None):
    """Prepare markdown item for TABLEBENCH tasks, supports column_header + row_header table structure"""
    table_data = None
    column_header = []
    row_header = []
    question = None
    ...
    # Format original header information for display in prompt
...
def process_aitqa_data_batch(input_file, output_file, model_config, log_file, max_tokens=2048,
                         temperature=0.0, tensor_parallel_size=1, start_from=0, batch_size=32):
    logger = setup_logger(log_file)
    start_time = time.time()
    logger.info(f"Started processing AI-TQA data: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    ...
    for epoch in range(max_func_call):
        ...
        try:
            ...
            for orig_idx, response in zip(prompt_indices, responses):
                ...
                if combined_token_count > max_tokens_allowed - 1000:  # Leave space for one response
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Process AI-TQA dataset with batch inference')
    
    parser.add_argument('--input_file', type=str, help='Path to input file (defaults to test.jsonl in data/aitqa)')
