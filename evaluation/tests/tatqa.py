...
def process_tatqa_data_batch(input_file, gold_file, output_file, model_path, log_file, max_tokens=2048, 
    ...
    remaining_items = [item for item in expanded_items if item["id"] not in processed_ids]
    logger.info(f"Remaining items to process: {len(remaining_items)}/{len(expanded_items)}")
        ...
        try:
            ...
            for i, (item, response) in enumerate(zip(current_batch, responses)):
                ...
                global_item_index = batch_start + i + 1
                 ...
                 # 记录详细的处理信息
                logger.info(f"Processing item {global_item_index}/{len(expanded_items)}... [ID: {item_id}]")
    ...
    average_time_per_item = total_time / (len(expanded_items) - len(processed_ids)) if (len(expanded_items) - len(processed_ids)) > 0 else 0
...
def main():
    ...
    # Setup paths
    input_file = args.data_path or os.path.join(base_path, "data/tatqa/tatqa_dataset_test.json")
    gold_file = args.gold_file or os.path.join(base_path, "data/tatqa/tatqa_dataset_test_gold.json")
