...
def process_tabfact_data_batch(input_file, output_file, model_path, log_file, max_tokens=2048,
    ...
    logger.info(f"Started processing TabFact data: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        ...
        try:
            ...
            for i, (item, response) in enumerate(zip(current_batch, responses)):
                ...
                if not extracted_answer:
                    logger.warning(f"Unable to extract label from answer, using 'UNKNOWN'")
                ...
                if (global_item_index % 500 == 0):
                    ...
                    accuracy = correct_count / (success_count + error_count) if (success_count + error_count) > 0 else 0
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Process TabFact dataset with VLLM batch inference')
