...
def process_tablebench_data_batch(input_file, output_file, model_config, log_file, max_tokens=2048,
    ...
    logger.info(f"Started processing TableBench data: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Process TableBench dataset with batch inference')
    
    parser.add_argument('--input_file', type=str, help='Path to input file (defaults to test.jsonl in data/tablebench)')
