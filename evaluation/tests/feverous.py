...
def process_feverous_data_batch(input_file, output_file, model_path, log_file, max_tokens=2048,
        ...
        try:
            ...
            for i, (item, response) in enumerate(zip(current_batch, responses)):
                ...
                if not extracted_answer:
                    logger.warning(f"Unable to extract label from answer, using 'NOT_EXTRACTED'")
...
def parse_arguments():
    parser = argparse.ArgumentParser(description='Process FEVEROUS dataset with VLLM batch inference')
